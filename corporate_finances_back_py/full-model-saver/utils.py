...
@debugger_wrapper('Error en la conexion local a base de datos','Error fatal conectando a base de datos desde el local')
def local_connect_to_db():
    f = open(r"C:\Users\JeissonBotache\.aws\aurora_sources8_dev.txt", "r")
    conn_string = f.read()
    logger.warning(f'El string de conexion es:\n{conn_string}')
    sql_engine = create_engine(conn_string)
    db_connection = sql_engine.connect()
    return db_connection